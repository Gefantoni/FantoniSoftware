...
router.post('/', autenticar, async (req, res) => {
  ...
  if (isNaN(mesNum) || mesNum < 1 || mesNum > 12) return res.status(400).json({ erro: 'Mês inválido (1-12)' });
  if (isNaN(anoNum) || anoNum < 2000 || anoNum > 2100) return res.status(400).json({ erro: 'Ano inválido (2000-2100)' });
